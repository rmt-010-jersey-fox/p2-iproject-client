<template>
    <div class="pcoded-inner-content">
                        ...
                        <div class="card-block table-border-style">
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-for="(data, id) in items.data" :key="id">
                                            <th> {{ id+1 }} </th>
                                            <th> {{ data.name }} </th>
                                            <th><img :src="data.imageUrl" class="itemCover" /></th>
        ...
        <modal :data="modalData" :types="'Banner'" :action="action"></modal>
...
export default {
  ...
  mounted () {
    this.$store.commit('setTitle', 'Banner Materials')
  },
  created () {
    try {
      this.$store.dispatch('getBannerAll')
    } catch (err) {
    ...
    showDisableModal (id, status) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store.dispatch('updateBannerStatus', data)
        }
      })
    },
    showDeleteModal (id) {
